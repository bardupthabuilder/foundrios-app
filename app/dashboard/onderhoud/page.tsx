...
const STATUS_COLORS: Record<string, string> = { active: 'bg-green-100 text-green-700', paused: 'bg-yellow-100 text-yellow-700', cancelled: 'bg-zinc-100 text-zinc-500' }
  ...
  return (
    <div className="p-4 lg:p-6 pt-16 lg:pt-6 max-w-4xl">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900">Onderhoud</h1>
          <p className="text-sm text-zinc-500">{activeContracts.length} actieve contracten · {fmt(Math.round(monthlyRecurring))}/mnd recurring · {upcomingVisits} bezoeken komende 30 dagen</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border-2 border-dashed border-zinc-200 p-12 text-center">
          ...
          <p className="mt-2 text-sm text-zinc-500">Nog geen onderhoudscontracten.</p>
        ...
        <div className="space-y-2">
          {contracts.map(c => (
            <div key={c.id} className="rounded-lg border p-4 hover:bg-zinc-50 transition-colors">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-500 mt-1">
                    ...
                    {c.next_visit && (
                      <span className={`flex items-center gap-0.5 ${new Date(c.next_visit) <= new Date() ? 'text-red-600 font-medium' : ''}`}>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => toggleStatus(c.id, c.status)} className="rounded-lg border p-2 text-zinc-400 hover:text-zinc-700 hover:bg-zinc-100" title={c.status === 'active' ? 'Pauzeren' : 'Hervatten'}>
          ...
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={createContract} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-medium text-zinc-600">Frequentie</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-zinc-600">Prijs per bezoek (€)</label>
                ...
                <div>
                  <label className="text-xs font-medium text-zinc-600">Eerste bezoek</label>
