...
const statusConfig: Record<string, { label: string; color: string }> = {
  draft: { label: 'Concept', color: 'bg-zinc-100 text-zinc-700' },
  sent: { label: 'Verstuurd', color: 'bg-blue-100 text-blue-700' },
  paid: { label: 'Betaald', color: 'bg-green-100 text-green-700' },
  overdue: { label: 'Verlopen', color: 'bg-red-100 text-red-700' },
  cancelled: { label: 'Geannuleerd', color: 'bg-zinc-100 text-zinc-500' },
}
...
export default function InvoiceDetailPage() {
  ...
  return (
    <div className="p-4 lg:p-6 pt-16 lg:pt-6 max-w-4xl">
      <button onClick={() => router.push('/dashboard/facturen')} className="mb-4 inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-700">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-6">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-zinc-900">{invoice.title || invoice.invoice_number || 'Factuur'}</h1>
            <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${isOverdue ? 'bg-red-100 text-red-700' : sc.color}`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <div className="rounded-lg border p-3">
          <div className="text-xs text-zinc-500">Bedrag excl. BTW</div>
          <div className="text-lg font-semibold">{fmt(invoice.amount_excl_vat)}</div>
        </div>
        <div className="rounded-lg border p-3">
          <div className="text-xs text-zinc-500">Incl. BTW ({invoice.vat_pct}%)</div>
        ...
        <div className={`rounded-lg border p-3 ${isOverdue ? 'border-red-200 bg-red-50' : ''}`}>
          <div className="text-xs text-zinc-500">Vervaldatum</div>
      ...
      <div className="mb-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="rounded-lg bg-zinc-50 p-3">
          <div className="text-xs text-zinc-500">Aangemaakt</div>
          <div className="text-sm">{new Date(invoice.created_at).toLocaleDateString('nl-NL')}</div>
        </div>
        <div className="rounded-lg bg-zinc-50 p-3">
          <div className="text-xs text-zinc-500">Factuurdatum</div>
        ...
        <div className="rounded-lg bg-zinc-50 p-3">
          <div className="text-xs text-zinc-500">Betaald op</div>
      ...
      {invoice.items && invoice.items.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold text-zinc-900 mb-3">Regels</h2>
            ...
            <table className="w-full text-sm">
              ...
              <tfoot>
                <tr className="border-t">
                  <td colSpan={4} className="pt-2.5 text-right text-xs text-zinc-500">Subtotaal</td>
                  <td className="pt-2.5 text-right font-semibold">{fmt(invoice.amount_excl_vat)}</td>
                </tr>
                <tr>
                  <td colSpan={4} className="py-1 text-right text-xs text-zinc-500">BTW ({invoice.vat_pct}%)</td>
      ...
      {invoice.projects && (
        <div className="mb-6 rounded-lg border p-4">
          <h3 className="text-xs font-medium text-zinc-500 mb-1">Project</h3>
          <button onClick={() => router.push(`/dashboard/projecten/${invoice.projects.id}`)} className="text-sm font-medium text-blue-600 hover:underline">
